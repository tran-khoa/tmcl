...
class EvalConfig:
    ...
    evaluated_tasks: tuple[int, ...] = ()
    eval_on_full_train: bool = True  # if False, evaluates only on labeled subset (--labeled_frac)
    knn_per_session: bool = False
    eval_onevsall: tuple[int, ...] = ()
    eval_valid: bool = True
    linear_eval: bool = True
    linear_last_n_layers: int | None = 4
    linear_resume: bool = False
    linear_lr: float = 0.1
    linear_batch_size: int = 1024
    linear_lr_scheduler: Literal['cosine', 'step'] = 'cosine'
...
class Config:
    ...
    cons_augments: Literal['simclr', 'cassle'] = 'cassle'
